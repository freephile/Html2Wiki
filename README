...
This extension officially lives at [https://www.mediawiki.org/wiki/Extension:Html2Wiki](https://www.mediawiki.org/wiki/Extension:Html2Wiki) This extension to MediaWiki is used to import HTML content (including images) into the wiki.
...
What, if any, additional tables could we want in the database? <sup>[[1]](#cite_note-1)</sup>
...
Logging is provided at [Special:Log/html2wiki](/wiki/index.php/Special:Log/html2wiki "Special:Log/html2wiki") The facility for logging will tap into `LogEntry` as outlined at [https://www.mediawiki.org/wiki/Manual:Logging\_to\_Special:Log](https://www.mediawiki.org/wiki/Manual:Logging_to_Special:Log)
...
-   @todo publish the extension upstream. first with the @link [http://www.mediawiki.org/wiki/Template:Extension](http://www.mediawiki.org/wiki/Template:Extension)

In order to use Parsoid at all, we need to have the content conform to the

MediaWikiDOMspec, which is based on HTML5 and RDFa [https://www.mediawiki.org/wiki/Parsoid/MediaWiki\_DOM\_spec\#Ref\_and\_References](https://www.mediawiki.org/wiki/Parsoid/MediaWiki_DOM_spec#Ref_and_References).

 We need to parse the incoming content, validate and possibly transform the document type to HTML5 (`<!DOCTYPE html>`) and then transform the HTML5 to MediaWiki DOMspec [http://www.w3.org/TR/html5/syntax.html\#html-parser](http://www.w3.org/TR/html5/syntax.html#html-parser)
...
@link [http://www.w3.org/TR/html5/obsolete.html](http://www.w3.org/TR/html5/obsolete.html)
...
-   mgc\_html v3.2\_2.03
-   infohub\_core v3.2\_2.02

Variables we care about
-----------------------

1.  We probably want a variable that can interact with the max upload size
2.  [\$wgMaxUploadSize](https://www.mediawiki.org/wiki/Manual:$wgMaxUploadSize)[\*] = 104857600 bytes (100 MB)
3.  [\$wgFileBlacklist](https://www.mediawiki.org/wiki/Manual:$wgFileBlacklist) we don't care about because we use our own file upload and mime detection
4.  [\$wgVisualEditorParsoidURL](https://www.mediawiki.org/wiki/Extension:VisualEditor) we can use for API requests to Parsoid
5.  [\$wgLegalTitleChars](https://www.mediawiki.org/wiki/Manual:$wgLegalTitleChars) we use to check for valid file naming
6.  [\$wgMaxArticleSize](https://www.mediawiki.org/wiki/Manual:$wgMaxArticleSize) default is 2048 KB, which may be too small?
7.  [\$wgMimeInfoFile](https://www.mediawiki.org/wiki/Manual:$wgMimeInfoFile) we don't yet use
8.  Also, how do imagelimits come into play? [http://localhost:8080/w/api.php?action=query&meta=siteinfo&format=txt](http://localhost:8080/w/api.php?action=query&meta=siteinfo&format=txt)
...
1.  [↑](#cite_ref-1) [https://www.mediawiki.org/wiki/Manual:Hooks/LoadExtensionSchemaUpdates](https://www.mediawiki.org/wiki/Manual:Hooks/LoadExtensionSchemaUpdates)
